...
type DNSZombieMappings struct {
	lock.Mutex
	deletes        map[netip.Addr]*DNSZombieMapping // map[ip]toDelete
...
func (zombies *DNSZombieMappings) ForceExpire(expireLookupsBefore time.Time, nameMatch *regexp.Regexp, prefix *netip.Prefix) (namesAffected []string) {
	zombies.Lock()
	defer zombies.Unlock()
	return zombies.forceExpireLocked(expireLookupsBefore, nameMatch, prefix)

}

func (zombies *DNSZombieMappings) forceExpireLocked(expireLookupsBefore time.Time, nameMatch *regexp.Regexp, prefix *netip.Prefix) (namesAffected []string) {
	...
	for _, zombie := range zombies.deletes {
		...
		// If prefix is provided, skip zombies with IPs outside the range
		if prefix != nil && !prefix.Contains(zombie.IP) {
			continue
		}
...
func (zombies *DNSZombieMappings) ForceExpireByNameIP(expireLookupsBefore time.Time, name string, ips ...netip.Addr) error {
	...
	for _, ip := range ips {
		...
		zombies.forceExpireLocked(expireLookupsBefore, re, &prefix)
	}
	return nil
}
...
type CIDRMatcherFunc func(ip netip.Addr) bool
